...
export const createResource = (store, pathParts = [], val = null) => {
  ...
  const proxy = new Proxy(store.get(path), {
    get: function(target, prop, receiver) {
      const immidiateDelegates = ["length"]
      const delegates = [Symbol.iterator, "push", "merge", "val", "delete", "map", "slice"]
      const immidiateDelegation = immidiateDelegates.find((delegate) => delegate.toString() === prop.toString())
      const delegation = delegates.find((delegate) => delegate.toString() === prop.toString())

      if (immidiateDelegation) {
        return this[immidiateDelegation]()
      }
