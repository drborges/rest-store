...
export class Store {
  ...
  map(path, fn) {
    this.mutate(pathKeys(path), actions.map(fn))
  }

  post(path, data) {
    this.mutate(pathKeys(path), actions.post(data))
  }

  put(path, data) {
    this.mutate(pathKeys(path), actions.put(data))
  }

  patch(path, data) {
    this.mutate(pathKeys(path), actions.patch(data))
  }

  delete(path) {
    const keys = pathKeys(path)
    this.mutate(keys, actions.delete(keys.pop()))
  }
}
